...
void Vtb_processor::eval_end_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+eval_end_step Vtb_processor::eval_end_step\n"); );
#ifdef VM_TRACE
    // Tracing
...
std::unique_ptr<VerilatedTraceConfig> Vtb_processor::traceConfig() const {
...
void Vtb_processor___024root__trace_decl_types(VerilatedVcd* tracep);

void Vtb_processor___024root__trace_init_top(Vtb_processor___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vtb_processor___024root__trace_decl_types(tracep);
    Vtb_processor___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}

VL_ATTR_COLD void Vtb_processor___024root__trace_register(Vtb_processor___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD void Vtb_processor::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vtb_processor::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace-vcd with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vtb_processor___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
